...
elif args.loss == 'MSPCA':
    criterion = losses.MultiSimilarityPCALoss().cuda()
elif args.loss == 'Contrastive':
    criterion = losses.ContrastiveLoss().cuda()
elif args.loss == 'ContrastivePCA':
    criterion = losses.Contrastive_PCALoss().cuda()
elif args.loss == 'Triplet':
    criterion = losses.TripletLoss().cuda()
elif args.loss == 'NPair':
    criterion = losses.NPairLoss().cuda()
elif args.loss == 'TripletPCA':
    criterion = losses.TripletPCALoss().cuda()
elif args.loss == 'TripletPCA2':
    criterion = losses.TripletPCA2Loss().cuda()
elif args.loss == 'Proxy_PCA_Anchor' :
    criterion = losses.Proxy_PCA_Anchor(nb_classes = nb_classes, sz_embed = args.sz_embedding, mrg = args.mrg, alpha = args.alpha).cuda()
            ...
            if(epoch==args.nb_epochs-1):
                ...
                for noise in CORRUPTIONS:
                    for i in range(1,6):
                        ...
                        dl_ev_c = torch.utils.data.DataLoader(
                            ev_dataset_c,
                            batch_size=args.sz_batch,
                            shuffle=False,
                            num_workers=args.nb_workers,
                        ...
                        print("Corruption : " + noise + ", Severity : " + str(i))
